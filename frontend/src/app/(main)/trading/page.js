...
export default function TradingPage() {
  ...
  const [amendPosId, setAmendPosId] = useState(null);
  const [amendSl, setAmendSl] = useState('');
  const [amendTp, setAmendTp] = useState('');
  ...
  useEffect(() => {
    ...
    const onClosed = (data) => {
      setClosingId(null);
      setFeedback({ ok: true, message: `Position closed${data.price ? ` @ ${data.price}` : ''}` });
      setTimeout(refreshPositions, 400);
    };
  ...
  const closePosition = (p) => {
    setClosingId(p.position_id || p.ticket);
    setFeedback(null);
    socketEventBus.closeTrade({
      positionId: p.position_id || p.ticket,
      symbol: p.symbol,
    });
    setTimeout(() => setClosingId(null), 2000);
  };

  const startAmend = (p) => {
    setAmendPosId(p.position_id || p.ticket);
    setAmendSl(p.stop_loss != null ? String(p.stop_loss) : '');
    setAmendTp(p.take_profit != null ? String(p.take_profit) : '');
  };

  const saveAmend = (p) => {
    if (amendSl === '' && amendTp === '') return;
    setFeedback(null);
    socketEventBus.amendTrade({
      positionId: p.position_id || p.ticket,
      symbol: p.symbol,
      sl: amendSl !== '' ? parseFloat(amendSl) : null,
      tp: amendTp !== '' ? parseFloat(amendTp) : null,
    });
    setAmendPosId(null);
    setAmendSl('');
    setAmendTp('');
    setTimeout(refreshPositions, 600);
  };
        ...
        <div className="lg:col-span-2 rounded-2xl p-6" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-default)' }}>
          ...
          <div className="space-y-3">
            {positions.map((p) => {
              ...
              const isEditing = amendPosId === posId;
              const isClosing = closingId === posId;
              return (
                <div key={posId} className="flex items-center justify-between flex-wrap gap-3 p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border-default)' }}>
                  <div className="flex items-center gap-3">
                  ...
                  {isEditing ? (
                    <div className="flex items-end gap-2">
                      <div>
                        <label className="block text-[10px] font-mono uppercase mb-1" style={{ color: 'var(--neon-ruby)' }}>SL</label>
                        <input className="input" type="number" step="0.0001" style={{ width: 90 }} value={amendSl} onChange={(e) => setAmendSl(e.target.value)} />
                      </div>
                      <div>
                        <label className="block text-[10px] font-mono uppercase mb-1" style={{ color: 'var(--neon-emerald)' }}>TP</label>
                        <input className="input" type="number" step="0.0001" style={{ width: 90 }} value={amendTp} onChange={(e) => setAmendTp(e.target.value)} />
                      </div>
                      <button className="px-2.5 py-2 rounded-lg text-xs font-mono font-bold" style={{ background: 'rgba(0,255,136,0.12)', border: '1px solid rgba(0,255,136,0.35)', color: 'var(--neon-emerald)' }} onClick={() => saveAmend(p)}>Save</button>
                      <button className="px-2.5 py-2 rounded-lg text-xs font-mono" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border-default)', color: 'var(--text-secondary)' }} onClick={() => setAmendPosId(null)}>✕</button>
                    </div>
                  ) : (
                    <div className="flex gap-3 font-mono text-xs items-center">
                      <div className="text-right">
                        <div className="text-[10px]" style={{ color: 'var(--text-tertiary)' }}>SL / TP</div>
                        <div style={{ color: 'var(--text-secondary)' }}>
                          {p.stop_loss != null ? <span style={{ color: 'var(--neon-ruby)' }}>{p.stop_loss}</span> : <span style={{ color: 'var(--text-tertiary)' }}>—</span>}
                          {' / '}
                          {p.take_profit != null ? <span style={{ color: 'var(--neon-emerald)' }}>{p.take_profit}</span> : <span style={{ color: 'var(--text-tertiary)' }}>—</span>}
                        </div>
                      </div>
                      <button
                        onClick={() => startAmend(p)}
                        title="Edit stop loss / take profit"
                        className="px-2 py-1.5 rounded-lg text-[11px] font-mono transition-colors"
                        style={{ background: 'rgba(0,229,255,0.08)', border: '1px solid rgba(0,229,255,0.25)', color: 'var(--neon-cyan)', cursor: 'pointer' }}
                      >
                        ✎ Edit
                      </button>
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-[10px]" style={{ color: 'var(--text-tertiary)' }}>{posId}</span>
                    <button
