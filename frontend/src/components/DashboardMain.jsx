...
import { useTradingStore, useSessionStore } from '@/store';
...
function Dashboard() {
  ...
  // Paper Trading State
  const [tradingMode, setTradingMode] = useState('live'); // 'paper' or 'live' — live default: reflect the broker ledger
  ...
  // Live-mode stats: pull /api/stats + /api/signals so Win Rate / Total
  // Profit / Signals Today reflect the broker-backed trade ledger rather
  // than staying neutral while risk-stats-update only carries drawdown.
  const refreshLiveStats = useCallback(async () => {
    ...
    try {
      ...
      if (typeof s.totalTrades !== 'undefined') {
        setStats({
      ...
      // REST unavailable — keep store values; socket events still populate.
    }
    try {
      const signalsRes = await fetch('/api/signals?count=1', { headers: { Authorization: `Bearer ${token}` } });
      if (!signalsRes.ok) return;
      const body = await signalsRes.json();
      if (body && typeof body.count === 'number') {
        setStats({ totalSignals: body.count });
      }
    } catch (e) {
      // REST unavailable — keep store value.
    }
  }, [setStats]);
  ...
  useEffect(() => {
    ...
    // Live-mode ledger stats on mount (Win Rate / Total Profit / Signals)
    refreshLiveStats();
    ...
    const unsubs = [
      ...
      // Trade closed (lifecycle sync or explicit close) — refresh ledger stats
      socketEventBus.on('trade:closed', (trade) => {
        refreshLiveStats();
        if (trade?.symbol) {
          addNotification('info', 'Position Closed', `${trade.symbol} closed${trade.price ? ` @ ${trade.price}` : ''}`);
        }
      }),
      ...
      <div className="min-h-screen" style={{ background: 'var(--bg-void)' }}>
        ...
        <section style={{ padding: 'var(--space-xl)', maxWidth: '1600px', margin: '0 auto' }}>
          <div className="stats-grid">
            ...
            <StatsCard
              label="Active Trades"
              value={tradingMode === 'paper' ? paperMetrics.openPositions : riskStats.openPositions}
              icon={Activity}
              onClick={() => handleMetricClick('Active Trades', tradingMode === 'paper' ? paperMetrics.openPositions : riskStats.openPositions, 'default', Activity)}
            />
            <StatsCard
              label="Total Profit"
              value={tradingMode === 'paper' ? Math.abs(paperMetrics.totalProfit) : Math.abs(stats.profit)}
              prefix={tradingMode === 'paper'
                ? (paperMetrics.totalProfit < 0 ? '-$' : '+$')
                : (stats.profit < 0 ? '-$' : '+$')}
              variant={tradingMode === 'paper'
                ? (paperMetrics.totalProfit >= 0 ? 'success' : 'danger')
                : (stats.profit >= 0 ? 'success' : 'danger')}
              icon={BarChart3}
              onClick={() => handleMetricClick('Total Profit', tradingMode === 'paper' ? paperMetrics.totalProfit : stats.profit, tradingMode === 'paper' ? (paperMetrics.totalProfit >= 0 ? 'success' : 'danger') : (stats.profit >= 0 ? 'success' : 'danger'), BarChart3)}
            />
            <StatsCard
              label="Signals Today"
