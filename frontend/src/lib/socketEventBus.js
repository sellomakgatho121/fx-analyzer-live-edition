 ...
 *                     trade-executed, trade:closed, trade-rejected,
 *                     risk-stats-update, risk-settings-updated,
 *                     llm-models-list, model-changed, notification,
 *                     mt5-status, broker-status, vibe-research-update,
 *                     positions-update, analysis:result
 *   client -> server: execute-trade, close-trade, amend-trade,
 *                     update-risk-settings, get-llm-models,
 *                     switch-llm-model, mt5-get-status, mt5-reconnect,
 *                     broker-positions, agent:analyze
 */
...
export function connectSocket(url, options = {}) {
  ...
  // Trade closed (position lifecycle sync or explicit close-trade fill)
  socket.on('trade:closed', (trade) => emitLocal('trade:closed', trade));
...
/** Close an open position (by positionId, or symbol fallback) via the engine. */
export function closeTrade({ positionId, symbol, volume } = {}) {
  if (!positionId && !symbol) {
    console.warn('[socket] closeTrade requires positionId or symbol');
    return false;
  }
  return emit('close-trade', { positionId, symbol, volume });
}

/** Amend SL/TP of an open position (by positionId, or symbol fallback). */
export function amendTrade({ positionId, symbol, sl, tp } = {}) {
  if (!positionId && !symbol) {
    console.warn('[socket] amendTrade requires positionId or symbol');
    return false;
  }
  return emit('amend-trade', { positionId, symbol, sl, tp });
}
