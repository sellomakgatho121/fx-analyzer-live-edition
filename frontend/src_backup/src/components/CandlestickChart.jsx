...
function updateTradeMarkers(chart, series, trades) {
    if (!chart || !series) return;
    try {
        const markers = tradesToMarkers(trades);
        chart.addSeriesMarkers(series, markers);
    } catch (e) {
...
export default function CandlestickChart({ symbol = 'EUR/USD', livePrice = null, onPriceUpdate, trades = [] }) {
    ...
    useEffect(() => {
        ...
        updateTradeMarkers(chart, candleSeries, trades);
    ...
    useEffect(() => {
        if (candleSeriesRef.current) {
            updateTradeMarkers(chartRef.current, candleSeriesRef.current, trades);
        }
    }, [trades]);
